...
export default function BotFactory() {
  ...
  const toggleAll = () => {
    const newVal = !formData.enableImage; // Toggle based on first value
    setFormData(prev => ({
      ...prev,
      enableImage: newVal,
      enableVision: newVal,
  ...
  const callAI = async (prompt: string, model = 'nova-fast') => {
    try {
      ...
      // 2. Fallback (Simulated here since we can't expose API key client side safely without backend proxy)
      // In a real deployed app, you'd use a robust proxy. For this Vercel deployment, we rely on the text endpoint.
...
`;
      ...
      zip.file("env.txt", envContent.trim()); // The crucial file
  ...
  return (
    <div className="min-h-screen p-4 md:p-8 max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-8">
      ...
      <div className="lg:col-span-7 space-y-8">
        ...
        {/* 3. Neural Configuration (Ranges & Toggles) */}
        <section className="bg-glass border border-white/5 p-6 rounded-2xl backdrop-blur-sm">
          ...
          {generated && (
            ...
            >
              ...
              <div className="bg-gradient-to-br from-gray-900 to-black border border-emerald-500/30 p-6 rounded-2xl relative overflow-hidden group">
                ...
                <p className="text-gray-400 text-sm mb-4">You need to collect 4 keys to make the bot alive. Add them to your <code className="bg-white/10 px-1 rounded text-white">env.txt</code> file.</p>
                <div className="space-y-2 text-sm">
                  <KeyLink label="Bot Token" url="https://discord.com/developers/applications" desc="Discord Dev Portal -> Bot -> Reset Token" />
                  <KeyLink label="Server ID" url="#" desc="Right click server -> Copy ID (Enable Dev Mode)" />
                  <KeyLink label="Pollinations Key" url="https://pollinations.ai" desc="For unlimited image generation" />
                </div>
              </div>
              ...
              <div className="bg-red-500/5 border border-red-500/20 p-5 rounded-xl flex items-start gap-4">
                ...
                <div>
                  ...
                  <p className="text-gray-400 text-xs mt-1">
                    Once you paste your keys into <b>env.txt</b>, you MUST rename the file to just <b className="text-white">.env</b> (dot env). The bot will not start otherwise.
                  </p>
                </div>
              </div>
              ...
              <div className="bg-glass border border-white/5 p-6 rounded-2xl">
                ...
                <div className="space-y-4">
                  <HostMethod title="Easy: Local PC" desc="Install Node.js. Run 'npm install' then 'npm start'. Bot dies when PC sleeps." />
                  <HostMethod title="Recommended: Railway" desc="Upload to GitHub. Connect Railway. Add variables. 24/7 online." />
                  <HostMethod title="Expert: VPS" desc="Upload files. Use PM2 to keep it running forever." />
                </div>
              </div>
...
const InputGroup = ({ label, val, set, ph }: any) => (
  <div>
